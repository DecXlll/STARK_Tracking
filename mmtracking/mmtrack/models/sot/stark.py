...
#from ...kneron.KL720_model_inference import kneron_run
# from ...kneron.utils.ExampleHelper import get_device_usb_speed_by_port_id
#from ...kneron.utils_PLUS_1_3_0.ExampleHelper import get_device_usb_speed_by_port_id

#from ...kneron.onnx_inference import onnx_inference
...
class Stark(BaseSingleObjectTracker):
    ...
    def __init__(self,
        ...
        # self.res128 = 'resnet_128x128.nef'
        # self.res320 = 'resnet_320x320.nef'
        self.res128 = '128.nef'
        # self.res320 = '320.nef'
        self.res320 = '320.nef'
        ...
        #self.scan_devices = kp.core.scan_devices()
        #print("kneron_device:",self.scan_devices.device_descriptor_list)
        #if(self.scan_devices.device_descriptor_list != []):
        #    self.kneron = kneron_run()
        #    print("kneron is connet")
        # self.o = onnx_inference()
    ...
    def init_weights(self):
        ...
        if self.with_backbone:
            ...
            # torch.save(self.backbone,"save_model/stark/base_backbone.pth")
        ...
        if self.with_head:
            ...
            # torch.save(self.head,"head_ST1.pth")

    def print_shape(self,patch,dict=bool(0),key="",Notes=""):
        '''
        patch:input is dect or gpu device tensor 
        dict:if true patch is dict  
        key:dict key word
        Notes:
        '''
        if(dict == False):
            patch_ = tuple(t.cpu().numpy() for t in patch)#'feat'.'mask'
            patch_ = np.asarray(patch_).copy()
            print(Notes+":",patch_.shape)
        else:
            patch_ = tuple(t.cpu().numpy() for t in patch[key])
            print(Notes+":",np.shape(patch_))
    ...
    def extract_feat(self, img):
        ...
        #if(self.scan_devices.device_descriptor_list != []):
        #    if(self.frame_id == 0):
        #        print("inference by GPU")
        #        feat = self.backbone(img)
        #    elif(self.frame_id == 1):
        #        self.kneron.upload_model(model_path=self.res320, from_flash=False)
        #    if(self.frame_id >= 1):
                # print("inference by GPU")
                # feat = self.backbone(img)
        #        print("inference by kneron")
        #        feat = self.kneron.inference_model(img,BypassPreProc=True)
        #else:
        #    feat = self.backbone(img)
        feat = self.backbone(img)
        feat = self.neck(feat)

        # feat_neck = tuple(t.cpu().numpy() for t in feat)
        # feat_neck = np.asarray(feat_neck).copy()
        # print("backbone output:",feat_backbone.shape,"neck output:",feat_neck.shape)
    ...
    def update_template(self, img, bbox, conf_score):
        ...
        for i, update_interval in enumerate(self.update_intervals):
            # print("i:",i,"update_interval:",update_interval,"update_intervals:",self.update_intervals,"num_extra_template:",self.num_extra_template)
    ...
    def track(self, img, bbox):
        ...
        #x_patch_ = tuple(t.cpu().numpy() for t in x_patch)#'feat'.'mask'
        #print(x_patch_)
        #x_patch_ = np.asarray(x_patch_).copy()
        #x_patch_ = np.transpose(x_patch_,(0,2,3,1))
        #print("x_patch_ shape",x_patch_.shape)
        # x_patch_ = cv2.cvtColor(x_patch_[0],cv2.COLOR_RGB2BGR).copy()        
        # cv2.imwrite(f"img{self.num}.bmp",x_patch_.squeeze(0))
        self.num = self.num+1
        #print(x_patch_)
        #cv2.imshow("x_patch",x_patch_[0])
