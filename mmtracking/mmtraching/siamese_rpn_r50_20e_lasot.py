...
model = dict(
    ...
    backbone=dict(
        ...
        init_cfg=dict(
            type='Pretrained',
            checkpoint=
            'https://download.openmmlab.com/mmtracking/pretrained_weights/sot_resnet50.model'
        )),
    neck=dict(
        type='ChannelMapper',
...
data = dict(
        ...
        dataset_cfgs=[
            dict(
                ...
                pipeline=[
                    ...
                    dict(
                        type='VideoCollect',
                        keys=['img', 'gt_bboxes', 'is_positive_pairs']),
                    dict(type='ConcatSameTypeFrames'),
                    dict(type='SeqDefaultFormatBundle', ref_prefix='search')
                ],
                split='train',
    ...
    val=dict(
        type='LaSOTDataset',
        ann_file='data/lasot/annotations/lasot_test_infos.txt',
